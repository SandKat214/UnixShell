 ...
 *
 * Builtins need to write to std streams with,
 *
...
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  /** Update PWD variable
   *  sources:
   *  https://pubs.opengroup.org/onlinepubs/9699919799.2008edition/functions/getcwd.html
   *  https://pubs.opengroup.org/onlinepubs/9699919799.2008edition/functions/pathconf.html
   */
  path_max = pathconf(".", _PC_PATH_MAX);
  if (path_max < 0) size = 1024;
  else size = path_max;
  ...
  if (vars_set("PWD", ptr) < 0) return -1;
...
builtin_exit(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  if (cmd->word_count == 2) {
    ...
    /* Invalid argument */
    if (*end || end == cmd->words[1] || val < 0 || val > INT_MAX) {
      dprintf(get_pseudo_fd(redir_list, STDERR_FILENO),
              "exit: `%s': %s\n",
              cmd->words[1],
              strerror(EINVAL));
    ...
    params.status = val;    /* Valid, set status */
  } else if (cmd->word_count > 2) {
    /* Too many arguments */ 
