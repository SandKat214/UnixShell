...
{
  int flags = 0;
  /* Each IO operator has specified behavior. Set the appropriate
   * file flags.
   *
   ...
   * POSIX 1.2008
   * 
   * Source: https://pubs.opengroup.org/onlinepubs/9699919799.2008edition/functions/open.html
   */
  switch (io_op) {
    case OP_LESSAND: /* <& */
    case OP_LESS:    /* < */
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
     ...
     */
    int const did_fork = !is_builtin || !is_fg; /* XTODO */
    if (did_fork) {
      /* fork */
      child_pid = fork();
    ...
    if (child_pid == 0) {
      if (is_builtin) {
        ...
        /* Restore signals to their original values when unixshell was invoked
         */
         ...
         */
        if (execvp(cmd->words[0], cmd->words) < 0) err(127, 0); /* Exec failure -- why might this happen? */
