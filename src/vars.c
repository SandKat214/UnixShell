...
is_valid_varname(char const *name)
{
  ...
  if (!isalpha(*name_ptr) && *name_ptr != '_') {
    return 0;
  }

  /* check remaining characters */
  for (; *++name_ptr != '\0';) {
    if (!isalnum(*name_ptr) && *name_ptr != '_') {
      return 0;
    }
  }

  gprintf("%s is valid variable name", name);
  return 1;
}
...
{
  /* Implement argument validation before tail-calling internal
